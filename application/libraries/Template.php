...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['parentTitle']	= $this->_subTitle;
	...
	public function set_metadata($name, $content, $type = 'meta')
	{
		...
		switch($type)
		{
			...
			case 'css':
				$this->_metadata[$content] = '<link rel="stylesheet" href="'.$content.'" type="text/css" />';
			break;
	...
	private function _find_view($view, array $data, $parse_view = TRUE)
	{
		...
		if ( ! empty($this->_theme))
		{
			foreach ($this->_theme_locations as $location)
			{
				...
				{
					//echo $location . $theme_view . self::_ext($theme_view). '- - '.(file_exists($location . $theme_view . self::_ext($theme_view))? 'TRUE':'FALSE')."<br />";
	...
	public function parentTitle()
	{
		...
		if (func_num_args() >= 1)
		{
			$subTitle_segments = func_get_args();
			$this->_subTitle = implode($this->_title_separator, $subTitle_segments);
		}
