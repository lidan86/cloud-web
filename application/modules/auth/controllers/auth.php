...
//		$this->load->library('ion_auth'); 
	...
	function index() {
		...
		//$admin = $this->ion_auth->in_group('admin'); echo $admin;
		$data['active'] = "users";
	...
	function login() {
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() != true)
			{
				...
				{
					$data['message'] = m('en', $this->ion_auth->errors());
				}
			}
		}
			...
			if($this->input->post()){
				...
				if ($this->form_validation->run() === TRUE)
				{
					if($this->ion_auth->update($user->id, $data)){
						...
						if(!$this->ion_auth->is_admin() and $passwordChanged){
							$this->ion_auth->logout();
	...
	function forgot_password()
	{
		...
		if($this->input->post()){
			...
			else
			{
				...
				if ( $this->config->item('identity', 'ion_auth') == 'username' ){
					$identity = $this->ion_auth->where('username', strtolower($this->input->post('email', TRUE)))->users()->row();
				}
				else
				{
					$identity = $this->ion_auth->where('email', strtolower($this->input->post('email', TRUE)))->users()->row();
				}
	...
	function deactivate($id = NULL)
	{
		...
		else
		{
			...
			if ($this->input->post('confirm', TRUE) == 'yes')
			{
				...
				if ($this->ion_auth->logged_in() && Modules::run('role/has_role', 'allow_to_deactivate_user'))
				{
	...
	function edit_group($id)
	{
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() === TRUE)
			{
				$group_update = $this->ion_auth->update_group($id, $this->input->post('group_name', TRUE), $this->input->post('group_description', TRUE));
	...
	function make_sure_is_logged_in() {
		...
		if (!$this->ion_auth->logged_in() && !in_array($this->router->method, $no_redirect)) {
	...
	function fullname(){
		...
		$button ="<a href='auth/edit/".$this->session->userdata('user_id')."'><span class='icon-key'></span>".$this->session->userdata('fullname')."</a>'" ;
		//$data['widget'] = '<span class="glyphicon glyphicon-user"></span> '.$this->session->userdata('fullname');
		//return anchor('auth/edit/'.$this->session->userdata('user_id'),'<span class="icon-key"></span> '.$this->session->userdata('fullname'));
		//$this->load->view('v_widget',$data);
		return $button;
	}
        function profile(){
                if ($this->uri->segment(4) == $this->session->userdata('user_id')){
                }else{
                    redirect('auth/profile/edit/'.$this->session->userdata('user_id'));
		}            
...
//                $crud->set_field_upload('file','_assets/uploads/files');
                $crud->columns('first_name','last_name','mobile','email');
                $crud->fields('first_name','last_name','mobile','email',
...
//				 ->display_as('file_pic','Upload Pic');
...
//                $crud->callback_after_update(array($this, 'after_update'));
                ...
                // meredirect langsung ke edit profile
...
//                $crud->unset_list();
                ...
                $output = $crud->render();
		$this->template->build('profile_index', $output);
