...
class Ion_auth_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$this->prefix = $this->config->item('prefix', 'ion_auth').'_';
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, email, last_login, m_job_role_id as job')
	...
	**/
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, '.$this->tables['users'].'.id, first_name,m_job_role_id, last_name, password, active, last_login, group_id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.discount')
		                  ->where($this->identity_column, $identity)
						  ->join($this->tables['groups'], $this->tables['users'].'.'.$this->join['groups'].' = '.$this->tables['groups'].'.id')
		                  ->limit(1)
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				// register user in the session User_Data
				unset($user->password);
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'group_description'					=> $user->description,
		    'discount'							=> $user->discount,
			'job_role'							=> $user->m_job_role_id,
		);
