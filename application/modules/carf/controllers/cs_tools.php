<?php
class cs_tools extends AdminController {
    ...
    public function index(){
        ...
        $crud = $this->new_crud();
                $crud->set_table('carf');
                $crud->set_subject('(CARF) - CS Tools');
                $crud->columns('project_name');
                $crud->unset_add();$crud->unset_delete();$crud->unset_list();
                $crud->unset_export();$crud->unset_print();
                $crud->unset_back_to_list();
        $crud->required_fields('cs_tools_needed_id');
                $crud->edit_fields('cs_tools_needed_id','cs_tools_other','flag_form','updated_at');
                $crud->display_as('cs_tools_needed_id','CS Tools Needed')
                        ->display_as('cs_tools_other','Please Mention');
        ...
        $crud->callback_before_update(array($this,'callback_before'));
                $crud->set_relation_n_n('cs_tools_needed_id','r_master_carf','m_cs_tools_needed','carf_id','cs_tools_needed_id','name');
                $crud->set_lang_string('update_success_message',
                 'Your data has been successfully stored into the database.<br/>Please wait while you are redirecting to the list page.
                 <script type="text/javascript">
                  window.location = "'.base_url().'carf/capman/index/edit/'.$this->uri->segment(5).'";
                 </script>
