<?php
class traffic_proj extends AdminController {
...
//                    }
        ...
        if($this->session->userdata['group_id'] == '6' || $this->session->userdata['group_id'] == '10') {
            $this->template->build('v_carf_project', $data);
        }else{
            $this->template->build('v_carf_project_not', $data);
        }
    }
    ...
    public function do_insert(){
        ...
        $months1 = $_POST['months1'];
        $total_voice1 = $_POST['total_voice1'];
        $total_sms1 = $_POST['total_sms1'];
        $months2 = $_POST['months2'];
        $total_voice2 = $_POST['total_voice2'];
        $total_sms2 = $_POST['total_sms2'];
        $total_data_peak_traffic = $_POST['total_data_peak_traffic'];
        $total_data_peak_traffic1 = $_POST['total_data_peak_traffic1'];
        $total_data_peak_traffic2 = $_POST['total_data_peak_traffic2'];
        $total_data_revenue = 0;
        $years = date('Y');

        $burn_rate = $_POST['burn_rate'];
        $burn_rate1 = $_POST['burn_rate1'];
        $burn_rate2 = $_POST['burn_rate2'];
        $churn_rate = $_POST['churn_rate'];
        $churn_rate1 = $_POST['churn_rate1'];
        $churn_rate2 = $_POST['churn_rate2'];
        ...
        $fields1 = array(
            ...
            'target_launch_date'=> urlencode($tld),
            'target_termination_date' => urlencode($ttd),
            'months' => urlencode($months1),
            'total_voice' => urlencode($total_voice1),
            'total_sms' => urlencode($total_sms1),
            'total_data_peak_traffic' => urlencode($total_data_peak_traffic1),
            'total_data_revenue' => urlencode($total_data_revenue),
            'burn_rate' => urlencode($burn_rate1),
            'churn_rate' => urlencode($churn_rate1),
            'years'=> urlencode($years)
        ...
        $fields2 = array(
            ...
            'target_launch_date'=> urlencode($tld),
            'target_termination_date' => urlencode($ttd),
            'months' => urlencode($months2),
            'total_voice' => urlencode($total_voice2),
            'total_sms' => urlencode($total_sms2),
            'total_data_peak_traffic' => urlencode($total_data_peak_traffic2),
            'total_data_revenue' => urlencode($total_data_revenue),
            'burn_rate' => urlencode($burn_rate2),
            'churn_rate' => urlencode($churn_rate2),
            'years'=> urlencode($years)
...
//set the url, number of POST vars, POST data
        curl_setopt($ch,CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST ,1);
        curl_setopt($ch,CURLOPT_POSTFIELDS, http_build_query($fields1));
    ...
    public function editData($id) {
        ...
        $data = array(
            ...
            'total_data_peak_traffic' => $row[0]['total_data_peak_traffic'],
             'burn_rate' => $row[0]['total_sms'],
            'churn_rate' => $row[0]['total_data_peak_traffic'],
