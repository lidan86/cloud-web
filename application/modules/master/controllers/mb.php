...
class Mb extends AdminController
{
    ...
    public function index()
    {
        $crud = new grocery_CRUD();
                $crud->set_table('m_modul_blackberry');
                $crud->set_subject('Modul Blackberry');
                $crud->set_theme('datatables');
                $crud->required_fields('name');
                $crud->columns('name');

                $crud->fields('name');
                $crud->display_as('name','Modul Blackberry');
...
//                if(!Modules::run('role/has_role', 'delete_mbe')){$crud->unset_delete();}
    ...
    function insert_callback($post_array) {
        $req = array(
            'action'        => "Create Modul Blackberry",
    ...
    function delete_callback($post_array) {
        $req = array(
            'action'        => "Delete Modul Blackberry",
            'user_name'   => $this->session->userdata('username'),
            'date'    => date('Y-m-d  H:i:s')
