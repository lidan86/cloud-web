		...
		for ($m=0; $m < count($menu_array); $m++) {
			...
			if($item->parent_id == 0){ //main item
				...
				if($m+1 < count($menu_array) and $menu_array[$m+1]->parent_id != 0){
					...
					$list .= "\t\t\t\t\t\t".'<a href="#" class="dropdown-toggle" data-toggle="dropdown">'.$item->title_en.' <b class="caret"></b><span class="'.$item->icon.'"></span></a>'."\n";
					$list .= "\t\t\t\t\t\t".'<ul class="dropdown-menu">'."\n";
					$ol = true;
					$class = '';
				} else {
					$list .= "\t\t\t\t\t".'<li class="'.$class.'"><a href="'.base_url().$item->url.'">'.$item->title_en.'<span class="'.$item->icon.'"></span></a></li>'."\n";
					$class = '';
				}
			} elseif ($item->parent_id > 0){ //Sub item
				...
				if($item->parent_id == $pId){
					$list .= "\t\t\t\t\t\t\t".'<li><a href="'.base_url().$item->url.'"><span class="'.$item->icon.'"> '.$item->title_en.'</span></a></li>'."\n";
...
//                $return='<li class="navbar-right"><a href="http://192.168.50.70/bajau/traffic_Distribution">Traffic Distribution<span class="icon-cloud"></span></a></li>';
		$return = "\t\t\t\t".'<ul class="nav navbar-nav navbar-right">
					<li>'.$fullname.'</li>
					<li><a href="'.base_url().'/logout">'.t('logout').'<span class="icon-off"></span></a></li>
					<li><a target="_blank" href="'.base_url().'help">'.t('help').'<span class="icon-book"></span></a></li>
                                        <li class=""><a href="'.base_url().'msg/bug"><span class="icon-bug" style="color: #CC0000"></span></a></li>
