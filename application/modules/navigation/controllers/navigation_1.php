<?php
class Navigation extends GlobalController {
		...
		for ($m=0; $m < count($menu_array); $m++) {
			...
			if($item->parent_id == 0){ //main item
				...
				if($m+1 < count($menu_array) and $menu_array[$m+1]->parent_id != 0){
					...
					$list .= "\t\t\t\t\t".'<li class="'.$class.'"><a href="'.base_url().$item->url.'">'.$item->title_en.'</a></li>'."\n";
					$class = '';
				}
			} elseif ($item->parent_id > 0){ //Sub item
				...
				if($item->parent_id == $pId){
					$list .= "\t\t\t\t\t\t\t".'<li><a href="'.base_url().$item->url.'">'.$item->title_en.'</a></li>'."\n";
	...
	public function get_other_nav(){
		...
		$return = "\t\t\t\t".'<ul class="nav navbar-nav navbar-right">
					<li>'.$fullname.'</li>
					<li><a href="'.base_url().'logout" class="bold">'.t('logout').'</a></li>
				</ul>';
